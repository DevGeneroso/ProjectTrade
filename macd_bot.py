...
import numpy as np
import os.path
import telegram #pip install pytho
import asyncio
import schedule
import time
...
while True:
    ...
    if os.path.isfile('itau.csv'):
        df_itau = pd.read_csv('itau.csv', delimiter=',')
    else:
    ...
    #Criar cidugi para verificar a compra e a venda
    df_itub_total['flag'] = ''
    ...
    #Token e id referente o Telegram
    ...
    schedule.every(3).minutes.do(funcao_acao)
    time.sleep(3)